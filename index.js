// ✅ Merged Full Discord Bot for MTA:SA Whitelist + Verification
...
const MTA_SERVER = {
  host: "89.42.88.252",
  port: 22005,
  auth: { user: "admin", pass: "YourSecurePassword123!" },
};
...
const commands = [
  ...
  new SlashCommandBuilder().setName("whitelist").setDescription("Add/update whitelist").addStringOption(option => option.setName("serial").setDescription("MTA Serial").setRequired(true)).addStringOption(option => option.setName("discord_id").setDescription("Discord ID").setRequired(true)),
  new SlashCommandBuilder().setName("unwhitelist").setDescription("Remove whitelist entry").addStringOption(option => option.setName("serial").setDescription("MTA Serial").setRequired(true)),
  new SlashCommandBuilder().setName("removeverification").setDescription("Remove verification").addStringOption(option => option.setName("discord_id").setDescription("Discord ID").setRequired(true))
];
  ...
  try {
    if (interaction.isCommand() && interaction.commandName === "apply") {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(0x2C2F33)
        .setDescription("1. Fill the form\n2. Wait for approval");
    ...
    if (interaction.isModalSubmit() && interaction.customId === "whitelist_application") {
      ...
      if (!/^[a-fA-F0-9]{32}$/.test(serial)) {
        return interaction.reply({ content: "❌ Invalid serial.", ephemeral: true });
      }
      const [existing] = await conn.execute("SELECT * FROM whitelist_submissions WHERE discord_id = ?", [interaction.user.id]);
      if (existing.length && existing[0].retries >= 1) {
        return interaction.reply({ content: "❌ You've already reapplied once.", ephemeral: true });
      }
      await conn.execute("DELETE FROM whitelist_submissions WHERE discord_id = ?", [interaction.user.id]);
      await conn.execute(
        `INSERT INTO whitelist_submissions (discord_id, irl_name, irl_age, ingame_name, ingame_age, serial, retries)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [interaction.user.id,
         interaction.fields.getTextInputValue("irl_name"),
         interaction.fields.getTextInputValue("irl_age"),
         interaction.fields.getTextInputValue("ingame_name"),
         interaction.fields.getTextInputValue("ingame_age"),
         serial,
         existing.length ? 1 : 0]
      );
    ...
    if (interaction.isButton() && (interaction.customId.startsWith("accept_") || interaction.customId.startsWith("reject_"))) {
      ...
      if (!rows.length) return interaction.reply({ content: "❌ Not found.", ephemeral: true });
      if (interaction.customId.startsWith("accept_")) {
        await conn.execute(
          `INSERT INTO mta_whitelist (mta_serial, discord_id, added_by) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE discord_id=VALUES(discord_id)`,
          [rows[0].serial, rows[0].discord_id, interaction.user.tag]
        );
    ...
    if (interaction.isCommand() && interaction.commandName === "whitelistinfo") {
      ...
      if (!rows.length) return interaction.reply("No whitelisted users.");
    ...
    if (interaction.isCommand() && interaction.commandName === "mtaverify") {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription("Click below to verify your MTA account.")
    ...
    if (interaction.isButton() && interaction.customId === "verify_mta") {
      ...
      const [whitelist] = await conn.execute("SELECT mta_serial FROM mta_whitelist WHERE discord_id = ? LIMIT 1", [interaction.user.id]);
      if (!whitelist.length) return interaction.editReply("❌ Not whitelisted.");
      ...
      return interaction.editReply("✅ Verified for 5 minutes!");
    }
    ...
    if (interaction.isCommand() && interaction.commandName === "whitelist") {
      ...
      return interaction.reply(`✅ ${serial} whitelisted.`);
    }
    ...
    if (interaction.isCommand() && interaction.commandName === "unwhitelist") {
      ...
      return interaction.reply(`✅ ${serial} removed.`);
    }
    ...
    if (interaction.isCommand() && interaction.commandName === "removeverification") {
      ...
      return interaction.reply(`✅ Removed verification.`);
    }
  } catch (e) {
